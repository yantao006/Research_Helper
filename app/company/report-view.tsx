...
export default function ReportView({
  ...
  const activeIndex =
    run.docs.findIndex((doc) => doc.id === activeTab) >= 0
      ? run.docs.findIndex((doc) => doc.id === activeTab)
      : 0;
      ...
      <section className="panel docs-surface">
        <div className="docs-layout">
          ...
          <article className="tab-panel docs-main">
            ...
            {seoSection.keywords.length > 0 ? (
              <>
                <nav className="doc-keyword-jumps" aria-label="关键词命中导航">
                  <span className="meta">页内关键词命中</span>
                  {seoSection.keywords.map((keyword) => {
                    const anchor = highlightResult.firstAnchorByKeyword.get(keywordKey(keyword));
                    return (
                      <a key={keyword} className="doc-keyword-chip" href={anchor ? `#${anchor}` : "#"}>
                        {keyword}
                      </a>
                    );
                  })}
                </nav>
                <nav className="doc-keyword-jumps" aria-label="关键词专题内链">
                  <span className="meta">关键词专题页</span>
                  {seoSection.keywords.map((keyword) => (
                    <Link
                      key={`topic-${keyword}`}
                      className="doc-keyword-chip"
                      href={`/topic/${encodeURIComponent(keyword)}`}
                    >
                      {keyword}
                    </Link>
                  ))}
            ...
            <section className="panel" style={{ marginTop: "18px", padding: "16px" }}>
              <h2 style={{ marginTop: 0 }}>研究方法与边界</h2>
              <p className="meta">更新时间：{run.date}</p>
              <ul>
                <li>数据来源：公司公告、财报、公开新闻与第三方可验证公开信息。</li>
                <li>分析方法：基于结构化问题模板与统一口径进行对比分析，输出可追踪结论。</li>
                <li>边界说明：本页面用于研究交流，不构成任何投资建议。</li>
