...
async function searchLocalKeywordHits(query: string, limit: number): Promise<KeywordHit[]> {
...
export async function searchKeywordHits(query: string, limit = 20): Promise<KeywordHit[]> {
  const q = query.trim();
  if (!q) return [];

  if (hasPostgresDsn()) {
    try {
      return await searchKeywordHitsFromDb(q, limit);
    } catch (error) {
      console.error("keyword-search db query failed, fallback to local files", error);
    }
  }

  return searchLocalKeywordHits(q, limit);
}
